...
def imagic(pretrained_model_name_or_path,
           output_dir,
           input_image,
           target_text,
           seed=3434554,
    ...
    text_encoder = CLIPTextModel.from_pretrained(pretrained_model_name_or_path, subfolder="text_encoder", use_auth_token=True)
    vae = AutoencoderKL.from_pretrained(pretrained_model_name_or_path, subfolder="vae", use_auth_token=True)
    unet = UNet2DConditionModel.from_pretrained(pretrained_model_name_or_path, subfolder="unet", use_auth_token=True)
    ...
    if args.scale_lr:
        learning_rate = (
            learning_rate * gradient_accumulation_steps * args.train_batch_size * accelerator.num_processes
        )
